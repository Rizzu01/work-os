...
export default function AuthGate({children}:{children:React.ReactNode}){
 ...
 useEffect(()=>{const client=supabaseBrowser();if(!client){setReady(true);return}let active=true;const check=async()=>{const{data}=await client.auth.getSession();if(!active)return;const session=data.session;setUserEmail(session?.user.email??"");if(!session){if(path!=="/auth")router.replace("/auth");else setReady(true);return}if(path==="/auth"){router.replace("/");return}if(path!=="/onboarding"){try{const workspace=await getUserWorkspace(client,session.user.id);if(!workspace){router.replace("/onboarding");return}}catch{router.replace("/onboarding");return}}setReady(true)};check();const{data:{subscription}}=client.auth.onAuthStateChange(()=>check());return()=>{active=false;subscription.unsubscribe()};},[path,router]);
