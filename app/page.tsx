...
export default function Home() {
  ...
  const filteredTasks = useMemo(() => { const q=query.trim().toLowerCase(); return q ? tasks.filter(t=>`${t.title} ${t.project} ${t.status} ${t.priority}`.toLowerCase().includes(q)) : tasks; }, [query]);
  return <div className="shell">
    <aside className="sidebar">
      ...
      <nav className="nav"><a className="active" href="/">Home</a><a href="/my-work">My Work</a><a href="/projects">Projects</a><a href="#">Reports</a><a href="#">Team</a></nav>
      <div className="nav-label" style={{marginTop:18}}>MANAGE</div>
      <nav className="nav"><a href="#">Inbox</a><a href="/calendar">Calendar</a><a href="#">Settings</a></nav>
    </aside>
    <main className="main">
      <header className="topbar"><div className="crumb">Workspace / Home</div><div className="top-actions"><input className="search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search tasks, projects..."/><div className="avatar">RK</div></div></header>
      <section className="content">
        <div className="headline"><div><div className="eyebrow">SATURDAY, AUGUST 29</div><h1>Good afternoon, Rizwan</h1><p>Here&apos;s what needs your attention today.</p></div><a className="primary" href="/projects">+ New task</a></div>
        ...
        <div className="workspace-grid">
          <section className="panel"><div className="panel-head"><div><strong>My work</strong> <span className="muted"> · 8 tasks</span></div><div className="top-actions"><button className="primary" onClick={()=>setView(view === "list" ? "board" : "list")}>{view === "list" ? "Board" : "List"}</button></div></div>
            {view === "list" ? filteredTasks.map((t,i)=><div className="task" key={i}><div className="check"/><div><div className="task-title">{t.title}</div><div className="project">{t.project}</div></div><div className={`priority ${t.priority.toLowerCase()}`}>{t.priority}</div><div><span className={`pill ${t.status === "In progress" ? "progress" : t.status === "Review" ? "review" : t.status === "Completed" ? "done" : ""}`}>{t.status}</span></div><div className="muted">{t.due}</div></div>) : <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:0}}>{["Not started","In progress","Review","Completed"].map(s=><div key={s} style={{borderRight:"1px solid #eee",minHeight:260}}><div className="panel-head"><strong>{s}</strong></div>{filteredTasks.filter(t=>t.status===s).map((t,i)=><div className="side-item" key={i}><strong>{t.title}</strong><div>{t.project} · {t.priority}</div></div>)}</div>)}</div>}
            {query && filteredTasks.length===0 && <div className="side-item">No tasks match “{query}”.</div>}
          </section>
